...
if ! pacman -Q ${pacman_packages[@]} &>/dev/null; then
    ...
    task_output $! "$STDERR_LOG_PATH" "Download and install dioxus related tools"
    [[ $? -ne 0 ]] && exit 1
fi

bash rust.sh || exit $?
cargo install ${cargo_packages[@]} >>"$STDOUT_LOG_PATH" 2>>"$STDERR_LOG_PATH" &
task_output $! "$STDERR_LOG_PATH" "Download and compile dioxus core tools"
[[ $? -ne 0 ]] && exit 1
